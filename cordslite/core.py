...
# %% auto #0
__all__ = ['DISCORD_WEB', 'depoch', 'DiscordClient', 'DiscordError', 'DiscordObject', 'Guild', 'ChannelType', 'html_table',
           'Channel', 'Channels', 'Guilds', 'Message', 'Messages', 'date2snowflake', 'Attachment', 'User', 'Member',
           'Members', 'lbl', 'Webhook', 'Webhooks']
...
# %% ../nbs/00_core.ipynb #1a6cd5b8
@patch(as_prop=True)
def is_text(self:Channel): return self.type in (ChannelType.GUILD_TEXT, ChannelType.GUILD_ANNOUNCEMENT)

@patch(as_prop=True)
def is_thread(self:Channel): return self.type in (ChannelType.ANNOUNCEMENT_THREAD, ChannelType.PUBLIC_THREAD, ChannelType.PRIVATE_THREAD)

@patch(as_prop=True)
async def parent(self:Channel): return await self._client.channel(self.parent_id)
...
# %% ../nbs/00_core.ipynb #b4901b8f
@patch(as_prop=True)
def content_type(self:Attachment): return self.get('content_type') or mimetypes.guess_type(self.filename)[0] or 'application/octet-stream'
...
async def save(self:Attachment,
    ...
    dest = Path(dest)
    if dest.is_dir(): dest /= self.filename
    dest.write_bytes(await self.fetch())
    return dest
...
async def me(self:DiscordClient):
    ...
    return User(await self._req('GET', '/users/@me'), self)

# %% ../nbs/00_core.ipynb #25652e7c
@patch(as_prop=True)
def is_bot(self:User): return self.get('bot', False)


# %% ../nbs/00_core.ipynb #3e79fc1d
@patch(as_prop=True)
def author(self:Message): return User(self['author'], self._client)

@patch(as_prop=True)
def mentions(self:Message): return [User(u, self._client) for u in self.get('mentions', [])]

@patch(as_prop=True)
def referenced(self:Message):
    ref = self.get('referenced_message')
    return Message(ref, self._parent) if ref else None

# %% ../nbs/00_core.ipynb #1a8f4ba5
def lbl(ch):
    l = "#" if ch.is_text else "🔊"
    l += ch.name
...
async def tree(self:Guild, include_members=True, member_limit=1000):
    ...
    for c in await self.channels():
        ...
        elif c.is_text or c.type == ChannelType.GUILD_VOICE: by_parent[c.parent_id].append(c)
...
# %% ../nbs/00_core.ipynb #30c907da
@patch
async def threads(self:Guild):
    "The guild's active threads"
    return self.coll('Channel', (await self('GET', f'/guilds/{self.id}/threads/active'))['threads'])

@patch
async def threads(self:Channel):
    "This channel's active threads"
    return Channels(t for t in await (await self.guild).threads() if t.parent_id==self.id)
